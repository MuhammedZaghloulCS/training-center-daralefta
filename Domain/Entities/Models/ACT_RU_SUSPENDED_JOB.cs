// <auto-generated> This file has been auto generated by EF Core Power Tools. </auto-generated>
#nullable disable
using System;
using System.Collections.Generic;

namespace del.Models;

public partial class ACT_RU_SUSPENDED_JOB
{
    public string ID_ { get; set; }

    public int? REV_ { get; set; }

    public string CATEGORY_ { get; set; }

    public string TYPE_ { get; set; }

    public bool? EXCLUSIVE_ { get; set; }

    public string EXECUTION_ID_ { get; set; }

    public string PROCESS_INSTANCE_ID_ { get; set; }

    public string PROC_DEF_ID_ { get; set; }

    public string ELEMENT_ID_ { get; set; }

    public string ELEMENT_NAME_ { get; set; }

    public string SCOPE_ID_ { get; set; }

    public string SUB_SCOPE_ID_ { get; set; }

    public string SCOPE_TYPE_ { get; set; }

    public string SCOPE_DEFINITION_ID_ { get; set; }

    public string CORRELATION_ID_ { get; set; }

    public int? RETRIES_ { get; set; }

    public string EXCEPTION_STACK_ID_ { get; set; }

    public string EXCEPTION_MSG_ { get; set; }

    public DateTime? DUEDATE_ { get; set; }

    public string REPEAT_ { get; set; }

    public string HANDLER_TYPE_ { get; set; }

    public string HANDLER_CFG_ { get; set; }

    public string CUSTOM_VALUES_ID_ { get; set; }

    public DateTime? CREATE_TIME_ { get; set; }

    public string TENANT_ID_ { get; set; }

    public virtual ACT_GE_BYTEARRAY CUSTOM_VALUES_ID_Navigation { get; set; }

    public virtual ACT_GE_BYTEARRAY EXCEPTION_STACK_ID_Navigation { get; set; }

    public virtual ACT_RU_EXECUTION EXECUTION_ID_Navigation { get; set; }

    public virtual ACT_RU_EXECUTION PROCESS_INSTANCE_ID_Navigation { get; set; }

    public virtual ACT_RE_PROCDEF PROC_DEF_ID_Navigation { get; set; }
}